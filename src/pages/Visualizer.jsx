...
export default function Visualizer({ user }) {
  ...
  const runCode = async () => {
    // Skip backend validation for built-in programs — they are trusted and already work.
    // Only validate user-edited code (when codeDirty is true).
    const freshErr = codeDirty ? await validateCodeWithBackend(code) : null;
